...
- Backend cross-feature tests: resume, approval gates, resource locks, crash recovery, concurrency and the watchdogs are now tested in combination rather than only in isolation — which is how all five of the defects above were found.
- `scripts/seed_demo.py` fills a database with a realistic large history for demos and performance work.
